document.addEventListener('DOMContentLoaded', () => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (targetTab === 'fonts-tab') {
        if (!fontConsentGranted) {
          ...
          lazyLoadOptionFonts(allGoogleFonts);
        }
      }
  ...
  let optionFontsLoaded = false;
  function lazyLoadOptionFonts(fonts) {
    if (optionFontsLoaded) return;
    optionFontsLoaded = true;

    // Build a single query parameters string for all families (replacing spaces with '+')
    const familiesParam = fonts.map(font => `family=${font.family.replace(/ /g, '+')}`).join('&');

    // Extract and deduplicate all characters from labels and families to build a minimal text subset
    const allText = fonts.map(font => (font.label || font.family) + font.family).join('');
  ...
  function requestFontConsent() {
    ...
    if (ok) {
      ...
      // Load all fonts for dropdown list
      lazyLoadOptionFonts(allGoogleFonts);
  ...
  function loadFonts() {
    fetch(chrome.runtime.getURL('data/google-fonts-jp.json'))
      ...
      .then(fonts => {
        ...
        // Lazy load web fonts when user interacts with the font select dropdown
        ...
        fontSelect.addEventListener('mouseenter', handleInteraction);
