(function () {
  ...
  let cachedImproveHomeEnable = false;
...
<section class="namikarotter-modal-panel">
 ...
 <div style="display:flex; flex:1; align-items:center; justify-content:center; padding:24px; text-align:center; color:var(--text-secondary, #475569); line-height:1.7;">
   <div>
     ...
     <div style="font-size:13px; margin-bottom:16px;">バージョン 0.2.5</div>
  ...
  function showUpdatePopup(version) {
    ...
    popup.innerHTML = `
      ...
      ">
        ...
        ">
          <div>
            <h4 style="margin: 0 0 4px; font-size: 13.5px; font-weight: 700; color: var(--text-primary);">・「ホームのヘッダー・タブを改良」プラグインの一時無効化</h4>
            <p style="margin: 0 0 0 10px; font-size: 12.5px; color: var(--text-secondary, #475569);">
              不具合の修正対応に伴い、「ホームのヘッダー・タブを改良」プラグインの画面反映を一時的に停止しました。修正完了までプラグイン一覧での操作も無効化（既存のオン/オフ設定値は維持）されています。
            </p>
          </div>
        </div>
